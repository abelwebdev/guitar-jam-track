...
export default function TracksGrid({ pageSize }: { pageSize: number }) {
  ...
  return (
    <div className="min-h-screen w-full max-w-screen-xl mx-auto px-4 sm:px-6">
      {/* Header */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {tracks
              ...
              .map((track) => (
                <Link
                  key={track.id}
                  href={`/track/${(track.title ?? "untitled").replace(/\s+/g, "_")}?id=${track.id}`}
                >
                  <div className="rounded-lg border p-4 hover:shadow-sm transition-shadow bg-white dark:bg-gray-800 cursor-pointer">
